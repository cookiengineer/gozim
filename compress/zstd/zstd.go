...
func shannonEntropyBits(b []byte) int {
	...
	return int(-entropy * float64(len(b)))
}

func log2(x float64) float64 {
	return ln(x) * 1.4426950408889634
}

func ln(x float64) float64 {
	// Simple iterative approximation of natural log.
	if x <= 0 {
		return 0
	}
	var result float64
	...
	// Taylor series for ln(1+u) where u = x-1, |u| <= 0.5 (after range reduction x is in [0.5, 2]).
	// After reduction, x is in [0.5, 2]. Use best approximation.
